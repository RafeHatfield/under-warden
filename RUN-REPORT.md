...
## THE DELIVERABLE — one build, on the SE (lane `art/xframe-207`, r001 INSTALL-LATEST)
- Five seats, five plants (objects-isometric, props-unrecognizable ×2, searchlight-edges,
  lambda-in-the-face; seat 1 missed cement-cap-shadowed and was re-drawn once, caught). Above the
  reference 0/5, not below 3/5 — the install bar. Five flips disposed; the two that are the walk's
  own questions PARKED with the rulings quoted (the Λ: a seat read the bar-end rope wraps as
  "feet resting on the pinned timber row" — the Λ's low bar sits at the height of the wall's
  binding beam; the X: "trestle", the cold seats' own hedge).
- **Verdict committed before the build** (7ef5d135). First build carried `+dirty` because its own
  log was being written into the tree — discarded, rebuilt from the clean sha with the log outside
  the tree. Identity on the handset: `commit=7ef5d135 review=GATED`. **VERIFIED ON DEVICE** —
  booted `tier1_props_review`, rig live, `east_faces=5(ground-mask)`, `prop_occluders=3`, first
  window 56.7 fps at render_cpu 0.80 ms. Logs: `tools/cast_shadows/evidence/deliverable/`.
